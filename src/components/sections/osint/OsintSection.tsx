...
export const OsintSection = () => {
  ...
  const triggerAnomaly = (type: 'minor' | 'major') => {
    if (type === 'major') {
      ...
      mutateWorldState({
        anomalyLevel: worldState.anomalyLevel + 0.15,
        entropy: worldState.entropy + 0.1,
        tension: worldState.tension + 0.05
      });
    } else {
      mutateWorldState({
        signalNoise: worldState.signalNoise + 0.05,
        entropy: worldState.entropy + 0.02
      });
    }
  };
